...
public class ResetBlocksCommandExecutedProcedure extends FindblocksModElements.ModElement {
	...
	public static void executeProcedure(java.util.HashMap<String, Object> dependencies) {
		...
		int wCount = 0; // wool count
		int cCount = 0; // concrete count
		...
		for (int indexX = -1 * radius; indexX < radius; indexX++) {
			for (int indexY = -1 * radius; indexY < radius; indexY++) {
				for (int indexZ = -1 * radius; indexZ < radius; indexZ++) {
					...
					if (world.getBlockState(new BlockPos(((int) x) + indexX , ((int) y) + indexY , ((int) z) + indexZ)).toString().contains("concrete")) { //If target block is any instance of concrete
						BlockPos pos = new BlockPos(((int) x) + indexX , ((int) y) + indexY , ((int) z) + indexZ);
						BlockState state = TrussBlockBlock.block.getDefaultState();
						world.setBlockState(pos, state, 3); //Set back to Truss block
						cCount++;
					}
				}
			}
		...
		if (mcserv != null) {
			mcserv.getPlayerList().sendMessage(new StringTextComponent(wCount + " Wool and " + cCount + " Concrete inside radius " + radius + " have been reset."));
		}
	}
}
