...
public class FindblocksProcedure extends FindblocksModElements.ModElement {
	...
	public static void executeProcedure(java.util.HashMap<String, Object> dependencies) {
		...
		if (dependencies.get("simtype") == null) {
			System.err.println("Failed to load dependency simtype for procedure Findblocks!");
			return;
		}
			...
			for (int indexY = -1 * radius; indexY < radius; indexY++) {
				for (int indexZ = -1 * radius; indexZ < radius; indexZ++) {
					if (((new ItemStack((world.getBlockState(new BlockPos(((int) x) + indexX , ((int) y) + indexY , ((int) z) + indexZ))).getBlock()))
						...
						//Printing Coordinates of Found Blocks for testing
						//System.out.println("index:" + count + " x: " + (x + indexX) + "  y: " + (y + indexY) + "  z: " + (z + indexZ));
		...
		int type = (int) dependencies.get("simtype");
		if (type == 0) {
			SPHCodeProcedure.executeProcedure(dependencies, FoundBlocks, loaded_blocks); //Run SPH Code with foundblocks
		}

		if (type == 1) {
			TrussCodeProcedure.executeProcedure(dependencies, FoundBlocks, loaded_blocks); // Run Truss code with foundBlocks
		}
	}
