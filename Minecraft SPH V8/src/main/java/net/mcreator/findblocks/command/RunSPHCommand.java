	...
	private int execute(CommandContext<CommandSource> ctx) {
		...
		if (entity != null) {
			...
			});
			{
				...
				$_dependencies.put("simtype", 0); // tells findblocks to run sph code procedure
				...
				if(cmdparams.get("shortlong") == null) {
					$_dependencies.put("shortlong",false);
				} else {
					if(cmdparams.get("shortlong").equals("long")) {
						$_dependencies.put("shortlong",true);
					} else {
						$_dependencies.put("shortlong",false);
					}
				}
