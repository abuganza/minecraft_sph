...
public class SPHCodeProcedure extends FindblocksModElements.ModElement {
	...
	static double h_threshold = 2.1; //define the initial threshold // INCREASE from 1.5 to 2.1
	static double alpha = 0.1; //hourglass control // CHANGED TO 0.1 // CHANGED to 0.05
	static double Ehg = 1e6; //hourglass control [Pa]
	//static double mat[] = new double[] {0,1000000,10000000};
	static double mat[] = new double[] {1,1e10,0.4}; // change from 0.4
	//static double eta = 0.2*mat[1]; //viscosity 
	//static double eta = 0.005*mat[1]; //viscosity was 0.01 when changed code
	...
	static double y_special = 59.0;
	static double z_special = -17.0;
	...
	public static void executeProcedure(java.util.HashMap<String, Object> dependencies, List<BlockPos> foundBlocks, List<Integer> loaded_blocks) {
		...
		if(neighborCheck){
			try { //Lines 105-123, 160-168, and 178-180 correspond to writing data to CSV 
				...
				int n_t_steps = 5000;
				double dt = 0.0001; // changed from 0.0015
				for (int ti = 0; ti < n_t_steps; ti++){
					...
					for (int pi = 0; pi < n_particles; pi++){
						...
						double step_factor = ((double) ti) * 3 / n_t_steps; //change load from 2 to 4
						if(step_factor > 1){
							step_factor = 1;
						}
						VectorObj Fext = new VectorObj(new double[][]{new double[] {0}, new double[] {-900*step_factor}, new double[] {0}});
						if(loaded_blocks.contains(pi)){
							Fext = new VectorObj(new double[][]{new double[] {0}, new double[] {-900*step_factor}, new double[] {0}});
						}
					...
					try {
						...
						appendStressToCSV(writer, particle_PP[special_block], particle_x[special_block], ti*dt, particle_Fint[special_block]);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
				...
				if((int) dependencies.get("stresspos") == 1) { //Position
					...
					mcserv.getPlayerList().sendMessage(new StringTextComponent("Ellapsed Time:" + timeElapsed + "s"));
	...
	public static void appendStressToCSV(PrintWriter writer, Matrix i_PP, VectorObj particle_x, double time, Matrix Fint){
		...
		mcserv.getPlayerList().sendMessage(new StringTextComponent("Deflection: " + deflect + " m"));
	...
	public static List<Integer> findBounds(List<BlockPos> foundBlocks, World world) { //Returns indices of blocks that have anything other than tepolium blocks or air directly below them
		...
		for(int i = 0; i < foundBlocks.size(); i++){
			...
			else if (northPosBlock.getBlock() != TepoliumBlockBlock.block.getDefaultState().getBlock() && northPosBlock.getBlock() != Blocks.AIR.getDefaultState().getBlock()) {
				r.add(i);
			}
			
			else if (northNegBlock.getBlock() != TepoliumBlockBlock.block.getDefaultState().getBlock() && northNegBlock.getBlock() != Blocks.AIR.getDefaultState().getBlock()) {
				r.add(i);
			}
			
			else if (eastPosBlock.getBlock() != TepoliumBlockBlock.block.getDefaultState().getBlock() && eastPosBlock.getBlock() != Blocks.AIR.getDefaultState().getBlock()) {
				r.add(i);
			}
			
			else if (eastNegBlock.getBlock() != TepoliumBlockBlock.block.getDefaultState().getBlock() && eastNegBlock.getBlock() != Blocks.AIR.getDefaultState().getBlock()) {
				r.add(i);
			}
		}
		return r;
	...
	public static VectorObj calculate_Fint(int pi, VectorObj[] particle_X, VectorObj[] particle_x, boolean[][] Neighbor_list, Matrix[] particle_Ainv, Matrix[] particle_PP, Matrix[] particle_FF) {
		...
		for(int pj = 0; pj < particle_X.length; pj++) {
			if(Neighbor_list[pi][pj]) {
				...
				Fint = new VectorObj(Fint.plus(particle_PP[pi].plus(particle_PP[pj]).mat_dot_vector(gradW.scale(0.5)).scale(Voli*Volj)).getData());
				//hourglass control
